...
import { View, Text, Pressable, TextInput, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard, ActivityIndicator, Image, ToastAndroid } from "react-native";
...
const PostContent = () => {
  ...
  async function newPostHandler() {
    ...
    const response = await fetch(`${ENDPOINT}/citizen/posts/create`, {
      method: "POST",
      headers: {
    ...
    if (data.detail === "Unauthorized") {
      ...
      Toast.show({
        type: "success",
        text1: "Success",
        text2: "Content Posted Successfully",
      });
    ...
    >
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <ScrollView contentContainerStyle={tw`mx-5 my-auto flex flex-col justify-center`}>
            <Text style={tw`text-3xl font-bold text-center mt-10`}>Post New Content</Text>
          <View style={tw`flex-col gap-y-5 mt-12`}>
            <View style={styles.container}>
              {image && <Image source={{ uri : image }} style={tw`w-32 h-32 rounded-full mb-4`} />}
              <Button style={tw`bg-[#123] rounded-md`} title="Add Photo" onPress={pickImage} />
            </View>
            <TextInput
